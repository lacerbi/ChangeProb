...
% r-transition matrix
r_trans = zeros(numel(r), numel(v), numel(r));
for idx_r = 1:numel(r)
    for idx_v1 = 1:numel(v)
        for idx_r1 = 1:numel(r)
            a = exp(v(idx_v1)) * r(idx_r);
            logkerna = (a - 1) * log(r(idx_r1));
            b = exp(v(idx_v1)) * (1 - r(idx_r));
            logkernb = (b - 1) * log(1 - r(idx_r1));
            betaln_ab = gammaln(a) + gammaln(b) - gammaln(a + b);
            r_trans(idx_r, idx_r1, idx_v1) = exp(logkerna + logkernb - betaln_ab);
        end
    end
end
% normalize
...
% v-transition matrix
v_trans = zeros(numel(v), numel(k), numel(v));
for idx_v = 1:numel(v)
    for idx_k = 1:numel(k)
        for idx_v1 = 1:numel(v)
            current_x = v(idx_v1);
            current_mean = v(idx_v);
            current_var = exp(2*k(idx_k));
            normalization_constant = 1/sqrt(2*pi*current_var);
            v_trans(idx_v, idx_k, idx_v1) = normalization_constant * exp(-.5 * (current_x - current_mean)^2 / current_var);
        end
    end
end
% normalize
...
for t = 2:NumTrials
    ...
    % update most recently observed data point and prior over r,v,k
    Cprev = C(t);
    prior = post_prob;
end
